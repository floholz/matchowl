...
func compute(app core.App) (map[string]any, error) {
	...
	var emailReachable, emailOptedOut, unverified int
	for _, u := range realUsers {
		...
		// Email delivery needs a verified address AND the master email switch
		// left on — count both barriers separately (keys overlap on purpose).
		emailOn := masterEmailOn(u.GetString("notifyPrefs"))
		if u.Verified() && emailOn {
			emailReachable++
		}
		if !emailOn {
			emailOptedOut++
...
// masterEmailOn reports whether the email channel's master switch is on.
// Mirrors notify's prefEnabledFromRaw: only an explicit prefs["*"]["email"]
// = false turns it off; absent/unparseable prefs default to ON (opt-out).
func masterEmailOn(raw string) bool {
	if raw == "" {
		return true
	}
