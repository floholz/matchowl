...
// pingKlipy makes the cheapest possible KLIPY call (one trending item) to
// check the key/service works. The body isn't parsed — status 200 is enough.
func pingKlipy(ctx context.Context) (latencyMs int64, err error) {
	endpoint := fmt.Sprintf("https://api.klipy.com/api/v1/%s/gifs/trending?per_page=1&page=1", klipyKey())
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, endpoint, nil)
	if err != nil {
		return 0, err
	}
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return 0, fmt.Errorf("klipy: status %d", resp.StatusCode)
	}
