...
func Register(app core.App, se *core.ServeEvent) {
	...
	// GET /api/admin/gif/status — GIF-API dashboard: configured flag, a live
	// KLIPY health ping, and how many GIFs were ever posted (soft-delete moves
	// the url into origGif, so deleted GIFs still count as sent).
	g.GET("/admin/gif/status", func(e *core.RequestEvent) error {
		if !users.IsAdmin(e.Auth) {
			return apis.NewForbiddenError("admin only", nil)
		}
		sent, err := app.CountRecords(messages, dbx.NewExp("gif != '' OR origGif != ''"))
		if err != nil {
			return err
		}
		out := map[string]any{"configured": klipyKey() != "", "gifsSent": sent}
		if klipyKey() != "" {
			ctx, cancel := context.WithTimeout(e.Request.Context(), 5*time.Second)
			defer cancel()
			health := map[string]any{"ok": true}
			if ms, err := pingKlipy(ctx); err != nil {
				health["ok"] = false
